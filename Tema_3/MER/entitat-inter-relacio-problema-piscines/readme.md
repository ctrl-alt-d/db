...
Un control d'aigua a un client consisteix, doncs, en aplicar una sèrie de tipus de tests a l'aigua que porta aquell client en un determinat moment. Cada control s'identifica amb un codi de control i s'anota en quin moment s'ha realitzat (no s'anota el moment en què es realitza cada test; s'anota el moment en què es realitza tot el control). També s'anoten els resultats de cada tipus de test realitzat (el tècnic decideix, segons les dades de la piscina, quins tipus de tests són adients). Associat al control hi ha un camp de comentaris on el tècnic pot fer anotacions. El control és sempre de l'aigua d'una sola piscina, hem d'anotar quina.
